...
class XPathBase
{
     ...
     * @return bool (bool)   TRUE: OK / FALSE: KO
     * @see             _evaluateStep()
     */
    public function _bracketsCheck($term)
    {
        $leng             = strlen($term);
        $brackets         = 0;
     ...
     * @return int (int)    This method returns -1 if no string was found,
     *                                 otherwise the offset at which the string was found.
     * @see                   _evaluateStep()
     */
    public function _searchString($term, $expression)
    {
     ...
     * @param     $term      (string) String in which the search shall take place.
     * @return array|false|string[] (array)  see above
     */
    public function _bracketExplode($separator, $term)
    {
     ...
     * @param int $offset
     * @return false|string (string) Substring from the original string before the delimiter.
     * @see                   _afterstr()
     */
    public function _prestr($string, $delimiter, $offset = 0)
    {
     ...
     * @param     $delimiter (string) String containing the delimiter to use.
     * @param int $offset
     * @return false|string (string) Substring from the original string after the delimiter.
     * @see                   _prestr()
     */
    public function _afterstr($string, $delimiter, $offset = 0)
    {
        $offset = ($offset < 0) ? 0 : $offset;
        // Return the substring.
     ...
     *
     * @param string $message (string) a textual error message default is ''
     * @param string $line    (int)    the line number where the error occured, use __LINE__
     * @param string $file
     * @see getLastError()
     */
    public function _setLastError($message = '', $line = '-', $file = '-')
    {
     ...
     * @param string $lineNumber (int)     line number given by __LINE__
     * @param string $file
     * @param bool   $terminate  (bool)    (default TURE) End the execution of this script.
     */
    public function _displayError($message, $lineNumber = '-', $file = '-', $terminate = true)
    {
        // Display the error message.
        $err = '<b>XPath error in ' . basename($file) . ':' . $lineNumber . '</b> ' . $message . "<br \>\n";
        $this->_setLastError($message, $lineNumber, $file);
        if (($this->properties['verboseLevel'] > 0) or ($terminate)) {
            echo $err;
        }
     ...
     * @param string $file
     */
    public function _displayMessage($message, $lineNumber = '-', $file = '-')
    {
        // Display the error message.
     ...
     */
    public function _beginDebugFunction($functionName)
    {
        $fileName = basename(__FILE__);
        static $color = ['green', 'blue', 'red', 'lime', 'fuchsia', 'aqua'];
        static $colIndex = -1;
        $colIndex++;
     ...
     * @author    Nigel Swinson <nigelswinson@users.sourceforge.net>
     */
    public function _closeDebugFunction($aStartTime, $returnValue = '')
    {
        echo '<HR>';
        if (isset($returnValue)) {
            if (is_array($returnValue)) {
                ...
                echo 'Return Value: "' . htmlspecialchars($returnValue, ENT_QUOTES | ENT_HTML5) . "\"\n";
            }
        }
     ...
     */
    public function _profileFunction($aStartTime, $alertString)
    {
     ...
     * @return string
     */
    public function _treeDump($node, $indent = '')
    {
        $out = '';
        ...
        if (!empty($node['childNodes'])) {
            $out .= $indent . "  ['childNodes'] (Size = " . count($node['childNodes']) . ")\n";
        ...
        if (empty($indent)) {
            return '<pre>' . htmlspecialchars($out, ENT_QUOTES | ENT_HTML5) . '</pre>';
        }
        return $out;
    }
...
class XPathEngine extends XPathBase
{
    ...
    public $emptyNode = [
        'name'       => '',       // The tag name. E.g. In <FOO bar="aaa"> it would be 'FOO'
        'attributes' => [],  // The attributes of the tag E.g. In <FOO bar="aaa"> it would be array('bar'=>'aaa')
        'childNodes' => [],  // Array of pointers to child nodes.
        'textParts'  => [],  // Array of text parts between the cilderen E.g. <FOO>aa<A>bb<B>cc</A>dd</FOO> -> array('aa','bb','cc','dd')
    ...
    public $nodeStack       = []; // The elements that we have still to close.
    public $parseStackIndex = 0;       // The current element of the nodeStack[] that we are adding to while
    // parsing an XML source.  Corresponds to the depth of the xml node.
    // in our input data.
    public $parseOptions        = []; // Used to set the PHP's XML parser options (see xml_parser_set_option)
    public $parsedTextLocation  = ''; // A reference to where we have to put char data collected during XML parsing
    public $parsInCData         = 0;      // Is >0 when we are inside a CDATA section.
    public $parseSkipWhiteCache = 0;   // A cache of the skip whitespace parse option to speed up the parse.
     ...
     */
    public function __construct($userXmlOptions = [])
    {
        parent::__construct();
     ...
     */
    public function setXmlOption($optionID, $value)
    {
     ...
     * @see XML parser functions in PHP doc
     */
    public function setCaseFolding($onOff = true)
    {
     ...
     * @see XML parser functions in PHP doc
     */
    public function setSkipWhiteSpaces($onOff = true)
    {
     ...
     */
    public function &getNode($absoluteXPath = '')
    {
        if ($absoluteXPath === '/') {
            $absoluteXPath = '';
        }
        return $this->nodeIndex[$absoluteXPath] ?? false;
    }
     ...
     */
    public function wholeText($absoluteXPath, $textPartNr = null)
    {
        $status = false;
        $text   = null;
        ...
        do { // try-block
            ...
            $textPartSize = count($this->nodeIndex[$absoluteXPath]['textParts']);
     ...
     *
     * @param string $absoluteXPath      (string) The address of the node you would like to export.
     *                                   If empty the whole document will be exported.
     * @param array  $hilightXpathList
     * @return false|string (mixed)  The Xml document marked up as HTML so that it can
     *                                   be viewed in a browser, including any XML headers.
     *                                   FALSE on error.
     * @see                     _export()
     */
    public function exportAsHtml($absoluteXPath = '', $hilightXpathList = [])
    {
     ...
     * @see                   _export()
     */
    public function exportAsXml($absoluteXPath = '', $xmlHeader = null)
    {
     ...
     */
    public function exportToFile($fileName, $absoluteXPath = '', $xmlHeader = '<?xml version="1.0"?>')
    {
        $status = false;
        do { // try-block
     ...
     *
     * @param string $absoluteXPath (string) (optional, default is root) The node we choose as top-node
     * @param null   $xmlHeader     (string) (optional) content before <root> (see text above)
     * @param string $hilightXpathList
     * @return false|string|string[] (mixed)  The xml string, or FALSE on error.
     */
    public function _export($absoluteXPath = '', $xmlHeader = null, $hilightXpathList = '')
    {
        ...
        // Cache this now
        $this->parseSkipWhiteCache = $this->parseOptions[XML_OPTION_SKIP_WHITE] ?? false;
     ...
     */
    public function _InternalExport($node)
    {
     ...
     *                                       (check getLastError())
     */
    public function importFromString($xmlString, $absoluteParentPath = '')
    {
        $bDebugThisFunction = false;
        ...
        do { // try-block
            ...
            xml_set_elementHandler($parser, '_handleStartElement', '_handleEndElement');
            xml_set_character_dataHandler($parser, '_handleCharacterData');
            xml_set_defaultHandler($parser, '_handleDefaultData');
            xml_set_processing_instructionHandler($parser, '_handlePI');
     ...
     */
    public function _handleStartElement($parser, $nodeName, $attributes)
    {
        if (empty($nodeName)) {
        ...
        if ($this->bDebugXmlParse) {
            echo "<blockquote>" . htmlspecialchars("Start node: <" . $nodeName . ">", ENT_QUOTES | ENT_HTML5) . "<br>";
            echo "Appended to stack entry: $this->parseStackIndex<br>\n";
            echo "Text part before element is: " . htmlspecialchars($this->parsedTextLocation, ENT_QUOTES | ENT_HTML5);
            /*
      echo "<pre>";
     ...
     */
    public function _handleEndElement($parser, $name)
    {
        ...
        if ($this->bDebugXmlParse) {
            echo "Text part after element is: " . htmlspecialchars($this->parsedTextLocation, ENT_QUOTES | ENT_HTML5) . "<br>\n";
            echo htmlspecialchars("Parent:<{$this->parseStackIndex}>, End-node:</$name> '" . $this->parsedTextLocation, ENT_QUOTES | ENT_HTML5) . "'<br>Text nodes:<pre>\n";
            $dataPartsCount = count($this->nodeStack[$this->parseStackIndex]['textParts']);
            for ($i = 0; $i < $dataPartsCount; $i++) {
                echo "$i:" . htmlspecialchars($this->nodeStack[$this->parseStackIndex]['textParts'][$i], ENT_QUOTES | ENT_HTML5) . "\n";
            }
     ...
     */
    public function _handleCharacterData($parser, $text)
    {
        ...
        if ($this->bDebugXmlParse) {
            echo "Handling character data: '" . htmlspecialchars($text, ENT_QUOTES | ENT_HTML5) . "'<br>";
        }
        if ($this->parseSkipWhiteCache and !empty($text) and !$this->parsInCData) {
            ...
            if ($this->bDebugXmlParse) {
                echo "'Skip white space' is ON. reduced to : '" . htmlspecialchars($text, ENT_QUOTES | ENT_HTML5) . "'<br>";
            }
        }
     ...
     * @see       _handleStartElement(), _handleEndElement()
     */
    public function _handleDefaultData($parser, $text)
    {
        do { // try-block
            ...
            if ($this->bDebugXmlParse) {
                echo "Default handler data: " . htmlspecialchars($text, ENT_QUOTES | ENT_HTML5) . "<br>";
            }
            break; // try-block
        } while (false); // END try-block
    }
     ...
     * @return bool
     * @return bool
     * @see       PHP's manual "xml_set_processing_instructionHandler"
     */
    public function _handlePI($parser, $target, $data)
    {
     ...
     *                                     FALSE on error.
     */
    public function _internalAppendChild($stackParentIndex, $nodeName)
    {
    ...
    */
     ...
     */
    public function reindexNodeTree()
    {
        //return;
        $this->nodeIndex     = [];
    ...
    public function _recursiveReindexNodeTree($absoluteParentPath)
    {
        ...
        // Now start a reindex.
        $contextHash = [];
        $childSize   = count($parentNode['childNodes']);
    ...
    public function &cloneNode($node)
    {
        ...
        $childSize = count($node['childNodes']);
        for ($i = 0; $i < $childSize; $i++) {
     ...
     */
    public function match($xPathQuery, $baseXPath = '')
    {
        // Replace a double slashes, because they'll cause problems otherwise.
        static $slashes2descendant = [
     ...
     * @see match()
     */
    public function evaluate($xPathQuery, $baseXPath = '')
    {
     ...
     * @see                 evaluate()
     */
    public function _internalEvaluate($xPathQuery, $contextPath = '')
    {
        ...
        $xpathQueryList = (strpos($xPathQuery, '|') === false) ? [$xPathQuery] : $this->_bracketExplode('|', $xPathQuery);
        if ($bDebugThisFunction) {
        ...
        foreach ($xpathQueryList as $xQuery) {
            ...
            if (!$this->_bracketsCheck($xQuery)) {
                $this->_displayError(
                    'While parsing an XPath expression, in the predicate ' . str_replace($xQuery, '<b>' . $xQuery . '</b>', $xPathQuery) . ', there was an invalid number of brackets or a bracket mismatch.',
                    __LINE__,
                    __FILE__
                );
            }
     ...
     * @see                  evaluate()
     */
    public function _evaluateStep($contextPath, $steps)
    {
        // If you are having difficulty using this function.  Then set this to TRUE and
        // you'll get diagnostic info displayed to the output.
        $bDebugThisFunction = false;
        ...
        $xPathSet = []; // Create an empty array for saving the abs. XPath's found.
        ...
        if (is_array($contextPath)) {
            ...
            $contextPaths = [];   // Create an array to save the new contexts.
            ...
            if ($axis['axis'] == 'function') {
                ...
                if (!method_exists($this, $method)) {
                    $this->_displayError(
                        'While parsing an XPath expression, the axis ' . $axis['axis'] . ' could not be handled, because this version does not support this axis.',
                        __LINE__,
                        __FILE__
                    );
                }
     ...
     * @see                _evaluateStep()
     */
    public function _checkPredicates($xPathSet, $predicates, $nodeTest)
    {
        // If you are having difficulty using this function.  Then set this to TRUE and
        // you'll get diagnostic info displayed to the output.
        $bDebugThisFunction = false;
     ...
     * @see                   evaluate()
     */
    public function _evaluateFunction($function, $arguments, $absoluteXPath, $nodeTest = '')
    {
        // If you are having difficulty using this function.  Then set this to TRUE and
        // you'll get diagnostic info displayed to the output.
        $bDebugThisFunction = false;
        ...
        if (!method_exists($this, $method)) {
            ...
            $this->_displayError(
                "While parsing an XPath expression, " . "the function \"$function\" could not be handled, because this " . "version does not support this function.",
                __LINE__,
                __FILE__
            );
        }
     ...
     * @see                   evaluate()
     */
    public function _evaluatePredicate($absoluteXPath, $predicate, $nodeTest)
    {
        // If you are having difficulty using this function.  Then set this to TRUE and
        // you'll get diagnostic info displayed to the output.
        $bDebugThisFunction = false;
        ...
        do { // try-block
            ...
            if (!is_string($predicate)) {
                // Display an error message.
                $this->_displayError(
                    "While parsing an XPath expression " . "there was an error in the following predicate, " . "because it was not a string. It was a '" . $predicate . "'",
                    __LINE__,
                    __FILE__
                );
                $result = false;
            ...
            if ($predicate === '') {
                // Display an error message.
                $this->_displayError(
                    "While parsing an XPath expression " . "there was an error in the predicate " . "because it was the null string.  If you wish to seach " . "for the empty string, you must use ''.",
                    __LINE__,
                    __FILE__
                );
                $result = false;
            ...
            if ($stringDelimiterMismatsh > 0) {
                $this->_displayError(
                    "While parsing an XPath expression " . "there was an string delimiter miss match at pos [{$stringDelimiterMismatsh}] in the predicate string '{$predicate}'.",
                    __LINE__,
                    __FILE__
                );
                $result = false;
                ...
                switch ($operator) {
                    ...
                    case ' div ': // Return a division of the two results.
                        if ($right == 0) {
                            // Display an error message.
                            $this->_displayError(
                                'While parsing an XPath ' . 'predicate, a error due a division by zero ' . 'occured.',
                                __LINE__,
                                __FILE__
                            );
                        } else {
            ...
            if (preg_match(':\(:U', $predicate) && !preg_match(":text\(\)(\[\d*\])?$:", $predicate)) {
                ...
                if (!empty($after)) {
                    // Display an error message.
                    $this->_displayError(
                        'While parsing an XPath expression there was an error in the predicate ' . str_replace($predicate, '<b>' . $predicate . '</b>', $this->currentXpathQuery) . '. After a closing bracket there was something unknown: "' . $after . '"',
                        __LINE__,
                        __FILE__
                    );
                }
                ...
                if (empty($before) && empty($after)) {
                    ...
                    // Display an error message.
                    $this->_displayError(
                        'While parsing a predicate in an XPath expression, a function ' . str_replace($before, '<b>' . $before . '</b>', $this->currentXpathQuery) . ' was found, which is not yet supported by the parser.',
                        __LINE__,
                        __FILE__
                    );
                }
     ...
     * @see                 evaluate()
     */
    public function _checkNodeTest($contextPath, $nodeTest)
    {
        if ($nodeTest == '*') {
            ...
            switch ($function) {
                ...
                $this->_displayError(
                    'While parsing an XPath expression there was an undefined function called "' . str_replace($function, '<b>' . $function . '</b>', $this->currentXpathQuery) . '"',
                    __LINE__,
                    __FILE__
                );
            }
            ...
            $this->_displayError(
                "While parsing the XPath expression \"{$this->currentXpathQuery}\" " . "an empty and therefore invalid node-test has been found.",
                __LINE__,
                __FILE__
            );
        }
     ...
     * @see              _evaluateStep()
     */
    public function _getAxis($step, $nodePath)
    {
        ...
        if (!in_array($axis['axis'], array_merge($this->axes, ['function']))) {
            // Display an error message.
            $this->_displayError(
                'While parsing an XPath expression, in the step ' . str_replace($step, '<b>' . $step . '</b>', $this->currentXpathQuery) . ' the invalid axis ' . $axis['axis'] . ' was found.',
                __LINE__,
                __FILE__,
                false
            );
     ...
     * @see                 evaluate()
     */
    public function _handleAxis_child($axis, $contextPath)
    {
        $xPathSet = []; // Create an empty node-set to hold the results of the child matches
        if ($axis["node-test"] == "cdata") {
            if (!isset($this->nodeIndex[$contextPath]['textParts'])) {
                return '';
            }
            $tSize = count($this->nodeIndex[$contextPath]['textParts']);
            for ($i = 1; $i <= $tSize; $i++) {
     ...
     * @see                 evaluate()
     */
    public function _handleAxis_parent($axis, $contextPath)
    {
        $xPathSet = []; // Create an empty node-set.
     ...
     * @see                 evaluate()
     */
    public function _handleAxis_attribute($axis, $contextPath)
    {
        $xPathSet = []; // Create an empty node-set.
     ...
     * @see                 evaluate()
     */
    public function _handleAxis_self($axis, $contextPath)
    {
        $xPathSet = []; // Create an empty node-set.
     ...
     * @see                 evaluate()
     */
    public function _handleAxis_descendant($axis, $contextPath)
    {
        $xPathSet = []; // Create an empty node-set.
     ...
     * @see                 evaluate()
     */
    public function _handleAxis_ancestor($axis, $contextPath)
    {
        $xPathSet = []; // Create an empty node-set.
     ...
     * @see                 evaluate()
     */
    public function _handleAxis_namespace($axis, $contextPath)
    {
     ...
     * @see                 evaluate()
     */
    public function _handleAxis_following($axis, $contextPath)
    {
        $xPathSet = []; // Create an empty node-set.
        ...
        do { // try-block
            ...
            if ($position >= count($parent['childNodes'])) {
                break;
            } // try-block
            ...
            $nodeSize = count($nodeKeys);
            for ($k = 0; $k < $nodeSize; $k++) {
                if ($nodeKeys[$k] == $startXPath) {
                    break;
                } // Check whether this is the starting abs. XPath
            }
            for (; $k < $nodeSize; $k++) {
     ...
     * @see                 evaluate()
     */
    public function _handleAxis_preceding($axis, $contextPath)
    {
        $xPathSet = []; // Create an empty node-set.
     ...
     * @see                 evaluate()
     */
    public function _handleAxis_following_sibling($axis, $contextPath)
    {
        $xPathSet = []; // Create an empty node-set.
     ...
     * @see                 evaluate()
     */
    public function _handleAxis_preceding_sibling($axis, $contextPath)
    {
        $xPathSet = []; // Create an empty node-set.
     ...
     * @see                 evaluate()
     */
    public function _handleAxis_descendant_or_self($axis, $contextPath)
    {
        $xPathSet = []; // Create an empty node-set.
     ...
     * @see                 evaluate()
     */
    public function _handleAxis_ancestor_or_self($axis, $contextPath)
    {
        $xPathSet = []; // Create an empty node-set.
     ...
     * @param  $nodeTest
     * @return int (mixed)  Depending on the type of function being processed
     * @see                   evaluate()
     */
    public function _handleFunction_last($absoluteXPath, $arguments, $nodeTest)
    {
        // Calculate the size of the context.
        $parentNode = $this->nodeIndex[$absoluteXPath]['parentNode'];
        if ($nodeTest == "*") {
            $contextPos = count($parentNode['childNodes']);
        } elseif ($nodeTest == "cdata") {
            $absoluteXPath = substr($absoluteXPath, 0, strrpos($absoluteXPath, "/text()"));
            $contextPos    = count($this->nodeIndex[$absoluteXPath]['textParts']);
        } else {
            $contextPos = 0;
     ...
     * @param  $arguments     (string) String containing the arguments that were passed to the function.
     * @param  $nodeTest
     * @return false|int|mixed|string (mixed)  Depending on the type of function being processed
     * @see                   evaluate()
     */
    public function _handleFunction_position($absoluteXPath, $arguments, $nodeTest)
    {
     ...
     * @see                   evaluate()
     */
    public function _handleFunction_count($absoluteXPath, $arguments, $nodeTest)
    {
     ...
     * @param  $arguments     (string) String containing the arguments that were passed to the function.
     * @param  $nodeTest
     * @return array (mixed)  Depending on the type of function being processed
     * @see                   evaluate()
     */
    public function _handleFunction_id($absoluteXPath, $arguments, $nodeTest)
    {
     ...
     * @param  $arguments     (string) String containing the arguments that were passed to the function.
     * @param  $nodeTest
     * @return mixed (mixed)  Depending on the type of function being processed
     * @see                   evaluate()
     */
    public function _handleFunction_name($absoluteXPath, $arguments, $nodeTest)
    {
     ...
     * @param  $arguments     (string) String containing the arguments that were passed to the function.
     * @param  $nodeTest
     * @return mixed|string (mixed)  Depending on the type of function being processed
     * @see                   evaluate()
     */
    public function _handleFunction_string($absoluteXPath, $arguments, $nodeTest)
    {
        // Check what type of parameter is given
        if (preg_match('/^[0-9]+(\.[0-9]+)?$/', $arguments) or preg_match('/^\.[0-9]+$/', $arguments)) {
            $number = floatval($arguments); // Convert the digits to a number.
            return (string)$number; // Return the number.
     ...
     * @param  $arguments     (string) String containing the arguments that were passed to the function.
     * @param  $nodeTest
     * @return string (mixed)  Depending on the type of function being processed
     * @see                   evaluate()
     */
    public function _handleFunction_concat($absoluteXPath, $arguments, $nodeTest)
    {
        // Split the arguments.
     ...
     * @param  $arguments     (string) String containing the arguments that were passed to the function.
     * @param  $nodeTest
     * @return bool (mixed)  Depending on the type of function being processed
     * @see                   evaluate()
     */
    public function _handleFunction_starts_with($absoluteXPath, $arguments, $nodeTest)
    {
        // Get the arguments.
     ...
     * @see                   evaluate()
     */
    public function _handleFunction_contains($absoluteXPath, $arguments, $nodeTest)
    {
        // Get the arguments.
        $first  = trim($this->_prestr($arguments, ','));
        $second = trim($this->_afterstr($arguments, ','));
     ...
     * @param  $arguments     (string) String containing the arguments that were passed to the function.
     * @param  $nodeTest
     * @return false|string (mixed)  Depending on the type of function being processed
     * @see                   evaluate()
     */
    public function _handleFunction_substring_before($absoluteXPath, $arguments, $nodeTest)
    {
        // Get the arguments.
        $first  = trim($this->_prestr($arguments, ','));
        $second = trim($this->_afterstr($arguments, ','));
        ...
        $second = $this->_evaluatePredicate($absoluteXPath, $second, $nodeTest);
        // Return the substring.
        return $this->_prestr((string)$first, (string)$second);
    }
     ...
     * @see                   evaluate()
     */
    public function _handleFunction_substring_after($absoluteXPath, $arguments, $nodeTest)
    {
        // Get the arguments.
        $first  = trim($this->_prestr($arguments, ','));
        $second = trim($this->_afterstr($arguments, ','));
        ...
        $second = $this->_evaluatePredicate($absoluteXPath, $second, $nodeTest);
        // Return the substring.
        return $this->_afterstr((string)$first, (string)$second);
    }
     ...
     * @see                   evaluate()
     */
    public function _handleFunction_substring($absoluteXPath, $arguments, $nodeTest)
    {
        // Split the arguments.
        $arguments = explode(",", $arguments);
        $size      = count($arguments);
        ...
        if (!empty($arguments[2])) {
            return substr((string)$arguments[0], $arguments[1] - 1, $arguments[2]);
        } else {
            return substr((string)$arguments[0], $arguments[1] - 1);
        }
    }
     ...
     * @see                   evaluate()
     */
    public function _handleFunction_string_length($absoluteXPath, $arguments, $nodeTest)
    {
        $arguments = trim($arguments); // Trim the argument.
        // Evaluate the argument.
        $arguments = $this->_evaluatePredicate($absoluteXPath, $arguments, $nodeTest);
        return strlen((string)$arguments); // Return the length of the string.
    }
     ...
     * @param  $arguments     (string) String containing the arguments that were passed to the function.
     * @param  $nodeTest
     * @return string (stri)g trimed string
     * @see                    evaluate()
     */
    public function _handleFunction_normalize_space($absoluteXPath, $arguments, $nodeTest)
    {
        if (empty($arguments)) {
     ...
     * @see                   evaluate()
     */
    public function _handleFunction_translate($absoluteXPath, $arguments, $nodeTest)
    {
     ...
     * @see                   evaluate()
     */
    public function _handleFunction_boolean($absoluteXPath, $arguments, $nodeTest)
    {
        $arguments = trim($arguments); // Trim the arguments.
        // Check what type of parameter is given
     ...
     * @see                   evaluate()
     */
    public function _handleFunction_not($absoluteXPath, $arguments, $nodeTest)
    {
        $arguments = trim($arguments); // Trim the arguments.
     ...
     * @see                   evaluate()
     */
    public function _handleFunction_true($absoluteXPath, $arguments, $nodeTest)
    {
     ...
     * @param  $arguments     (string) String containing the arguments that were passed to the function.
     * @param  $nodeTest
     * @return false (mixed)  Depending on the type of function being processed
     * @see                   evaluate()
     */
    public function _handleFunction_false($absoluteXPath, $arguments, $nodeTest)
    {
     ...
     * @param  $arguments     (string) String containing the arguments that were passed to the function.
     * @param  $nodeTest
     * @return false|int (mixed)  Depending on the type of function being processed
     * @see                   evaluate()
     */
    public function _handleFunction_lang($absoluteXPath, $arguments, $nodeTest)
    {
        $arguments   = trim($arguments); // Trim the arguments.
     ...
     * @param  $arguments     (string) String containing the arguments that were passed to the function.
     * @param  $nodeTest
     * @return float|int (mixed)  Depending on the type of function being processed
     * @see                   evaluate()
     */
    public function _handleFunction_number($absoluteXPath, $arguments, $nodeTest)
    {
        ...
        if (is_numeric($arguments)) {
            return floatval($arguments); // Return the argument as a number.
     ...
     * @see                   evaluate()
     */
    public function _handleFunction_sum($absoluteXPath, $arguments, $nodeTest)
    {
        $arguments = trim($arguments); // Trim the arguments.
        ...
        for ($i = 0; $i < $size; $i++) {
            ...
            $sum   += floatval($value); // Add it to the sum.
        }
     ...
     * @param  $arguments     (string) String containing the arguments that were passed to the function.
     * @param  $nodeTest
     * @return false|float (mixed)  Depending on the type of function being processed
     * @see                   evaluate()
     */
    public function _handleFunction_floor($absoluteXPath, $arguments, $nodeTest)
    {
        if (!is_numeric($arguments)) {
            $arguments = $this->_evaluatePredicate($absoluteXPath, $arguments, $nodeTest);
        }
        $arguments = floatval($arguments); // Convert the arguments to a number.
     ...
     * @see                   evaluate()
     */
    public function _handleFunction_ceiling($absoluteXPath, $arguments, $nodeTest)
    {
        if (!is_numeric($arguments)) {
            $arguments = $this->_evaluatePredicate($absoluteXPath, $arguments, $nodeTest);
        }
     ...
     * @param  $arguments     (string) String containing the arguments that were passed to the function.
     * @param  $nodeTest
     * @return float (mixed)  Depending on the type of function being processed
     * @see                   evaluate()
     */
    public function _handleFunction_round($absoluteXPath, $arguments, $nodeTest)
    {
        if (!is_numeric($arguments)) {
            $arguments = $this->_evaluatePredicate($absoluteXPath, $arguments, $nodeTest);
        }
     ...
     */
    public function equalNodes($node1, $node2)
    {
    ...
    public function getNodePath($node)
    {
        ...
        $pathInfo = [];
        do {
            if (empty($node['name']) or empty($node['parentNode'])) {
                break;
            } // End criteria
            $pathInfo[] = ['name' => $node['name'], 'contextPos' => $node['contextPos']];
            $node       = $node['parentNode'];
        } while (true);

        $xPath = '';
        for ($i = count($pathInfo) - 1; $i >= 0; $i--) {
     ...
     *
     * @param $absoluteXPath
     * @return false|string (string) returns the absolute XPath of the parent
     */
    public function getParentXPath($absoluteXPath)
    {
     ...
     */
    public function hasChildNodes($absoluteXPath)
    {
        return (bool)(isset($this->nodeIndex[$absoluteXPath]) and count($this->nodeIndex[$absoluteXPath]['childNodes']));
    }
     ...
     * @param bool $reverse
     * @return string|string[] (string) The XML string with translated ampersands.
     */
    public function _translateAmpersand($xmlSource, $reverse = false)
    {
...
class XPath extends XPathEngine
{
     ...
     */
    public function __construct($fileName = '', $userXmlOptions = [])
    {
     ...
     */
    public function setModMatch($modMatch = XPATH_QUERYHIT_ALL)
    {
        switch ($modMatch) {
     ...
     * @return array|false|mixed (mixed) Array or single string of the names of the specified
     *                             nodes, or just the individual name.  If the node did
     *                             not exist, then returns FALSE.
     */
    public function nodeName($xPathQuery)
    {
     ...
     * @param bool $autoReindex      (bool)   (optional, default=TRUE) Reindex the document to reflect
     *                               the changes.  A performance helper.  See reindexNodeTree()
     * @return bool (bool)   TRUE on success, FALSE on error;
     * @see                 setModMatch(), reindexNodeTree()
     */
    public function removeChild($xPathQuery, $autoReindex = true)
    {
        $NULL               = null;
     ...
     */
    public function replaceChildByData($xPathQuery, $data, $autoReindex = true)
    {
        $NULL               = null;
        $bDebugThisFunction = false;  // Get diagnostic output for this function
        if ($bDebugThisFunction) {
     ...
     * @param bool $autoReindex      (bool)   (optional, default=TRUE) Reindex the document to reflect
     *                               the changes.  A performance helper.  See reindexNodeTree()
     * @return false|mixed (array)  The last replaced $node (can be a whole sub-tree)
     * @see                 reindexNodeTree()
     */
    public function replaceChild($xPathQuery, $node, $autoReindex = true)
    {
        $NULL = null;
        if (is_string($node)) {
     ...
     * @param      $node              (array)  A doc node.
     * @param bool $shiftRight        (bool)   (optional, default=TRUE) Shift the target node to the right.
     * @param bool $afterText         (bool)   (optional, default=TRUE) Insert after the text.
     * @param bool $autoReindex       (bool)   (optional, default=TRUE) Reindex the document to reflect
     *                                the changes.  A performance helper.  See reindexNodeTree()
     * @return bool (bool)   TRUE on success, FALSE on error.
     * @see                 _xml2Document(), appendChildByXml(), reindexNodeTree()
     */
    public function insertChild($xPathQuery, $node, $shiftRight = true, $afterText = true, $autoReindex = true)
    {
        if (is_string($node)) {
            if (!($node = $this->_xml2Document($node))) {
                return false;
     ...
     * @param      $node             (array)  A doc node.
     * @param bool $afterText        (bool)   (optional, default=FALSE) Insert after the text.
     * @param bool $autoReindex      (bool)   (optional, default=TRUE) Reindex the document to reflect
     *                               the changes.  A performance helper.  See reindexNodeTree()
     * @return false|mixed (bool)   TRUE on success, FALSE on error.
     * @see                 reindexNodeTree()
     */
    public function appendChild($xPathQuery, $node, $afterText = false, $autoReindex = true)
    {
        if (is_string($node)) {
            if (!($node = $this->_xml2Document($node))) {
                return false;
     ...
     * @param      $node             (array)  A doc node.
     * @param bool $afterText        (bool)   (optional, default=FLASE) Insert after the text.
     * @param bool $autoReindex      (bool)   (optional, default=TRUE) Reindex the document to reflect
     *                               the changes.  A performance helper.  See reindexNodeTree()
     * @return bool (bool)   TRUE on success, FALSE on error.
     * @see                 reindexNodeTree()
     */
    public function insertBefore($xPathQuery, $node, $afterText = true, $autoReindex = true)
    {
     ...
     * @param null $attrName           (string) (Optional) The name of the attribute. See above.
     * @return false|mixed (mixed)  hash-array or a string of attributes depending if the
     *                                 parameter $attrName was set (see above).  FALSE if the
     *                                 node or attribute couldn't be found.
     * @see                   setAttribute(), removeAttribute()
     */
    public function getAttributes($absoluteXPath, $attrName = null)
    {
        // Numpty check
        if (!isset($this->nodeIndex[$absoluteXPath])) {
     ...
     * @param bool $overwrite  (bool)   If the attribute is already set we overwrite it (see text above)
     * @return bool (bool)   TRUE on success, FALSE on failure.
     * @see                getAttribute(), removeAttribute()
     */
    public function setAttribute($xPathQuery, $name, $value, $overwrite = true)
    {
        return $this->setAttributes($xPathQuery, [$name => $value], $overwrite);
    }
     ...
     * @param bool $overwrite  (bool)   If the attributes are already set we overwrite them (see text above)
     * @return bool (bool)   TRUE on success, FALSE otherwise
     * @see                setAttribute(), getAttribute(), removeAttribute()
     */
    public function setAttributes($xPathQuery, $attributes, $overwrite = true)
    {
        $status = false;
        do { // try-block
     ...
     *
     * @param      $xPathQuery (string) xpath to the node (See note above).
     * @param null $attrList   (mixed)  (optional) if not set will delete *all* (see text above)
     * @return bool (bool)   TRUE on success, FALSE if the node couldn't be found
     * @see                 getAttribute(), setAttribute()
     */
    public function removeAttribute($xPathQuery, $attrList = null)
    {
        // Check for a valid xPathQuery
        $xPathSet = $this->_resolveXPathQuery($xPathQuery, 'removeAttribute');

        if (!empty($attrList) and is_string($attrList)) {
            $attrList = [$attrList];
        }
        ...
        foreach ($xPathSet as $absoluteXPath) {
            ...
            if ($attrList[0] === null) {
                $this->nodeIndex[$absoluteXPath]['attributes'] = [];
                continue;
            }
     ...
     *
     * @param  $absoluteXPath          (string) Full xpath OR a xpath-query that results to *one* xpath.
     * @return false|string (mixed)  The returned string (see above), FALSE if the node
     *                                 couldn't be found or is not unique.
     * @see                   getDataParts()
     */
    public function getData($absoluteXPath)
    {
     ...
     *
     * @param  $absoluteXPath            (string) Full xpath OR a xpath-query that results to *one* xpath.
     * @return false|mixed (mixed)  The returned array (see above), or FALSE if node is not
     *                                   found or is not unique.
     * @see                     getData()
     */
    public function getDataParts($absoluteXPath)
    {
     ...
     * @param int  $offset        (int)    (optional, default is 0) Starting offset. (Just like PHP's substr())
     * @param null $count         (number) (optional, default is ALL) Character count  (Just like PHP's substr())
     * @return false|string (mixed)  The sub string, FALSE if not found or on error
     * @see                   XPathEngine::wholeText(), PHP's substr()
     */
    public function substringData($absoluteXPath, $offset = 0, $count = null)
    {
     ...
     * @param int $offset      (int)    (optional, default is 0) Starting offset. (Just like PHP's substr_replace ())
     * @param int $count       (number) (optional, default is 0=ALL) Character count  (Just like PHP's substr_replace())
     * @param int $textPartNr  (int)    (optional) (see _getTextSet() )
     * @return bool (bool)   The new string value on success, FALSE if not found or on error
     * @see                   substringData()
     */
    public function replaceData($xPathQuery, $replacement, $offset = 0, $count = 0, $textPartNr = 1)
    {
     ...
     * @param int $offset     (int)    (optional, default is 0) Offset at which to insert the data.
     * @return bool (bool)   The new string on success, FALSE if not found or on error
     * @see                replaceData()
     */
    public function insertData($xPathQuery, $data, $offset = 0)
    {
     ...
     * @param int $textPartNr (int)    (optional, default is 1) (see _getTextSet())
     * @return bool (bool)   TRUE on success, otherwise FALSE
     * @see                 _getTextSet()
     */
    public function appendData($xPathQuery, $data, $textPartNr = 1)
    {
        if (!($textSet = $this->_getTextSet($xPathQuery, $textPartNr))) {
            return false;
        }
     ...
     * @param int $offset     (int)    (optional, default is 0) Starting offset. (Just like PHP's substr_replace())
     * @param int $count      (number) (optional, default is 0=ALL) Character count.  (Just like PHP's substr_replace())
     * @param int $textPartNr (int)    (optional, default is 0) the text part to delete (see _getTextSet())
     * @return bool (bool)   TRUE on success, otherwise FALSE
     * @see                _getTextSet()
     */
    public function deleteData($xPathQuery, $offset = 0, $count = 0, $textPartNr = 1)
    {
        if (!($textSet = $this->_getTextSet($xPathQuery, $textPartNr))) {
            return false;
        }
     ...
     * @param bool $reverse        (bool)  If TRUE entities will be encoded rather than decoded, ie
     *                             < to &lt; rather than &lt; to <.
     * @return array|string (mixed) The string or array returned with entities decoded.
     */
    public function decodeEntities($encodedData, $reverse = false)
    {
        static $aEncodeTbl;
        static $aDecodeTbl;
     ...
     * @return false|mixed (&array)  a node-tree
     */
    public function _xml2Document($xmlString)
    {
        $xmlOptions = [
     ...
     *
     * @param     $xPathQuery        (string) xpath to the node (See note above).
     * @param int $textPartNr        (int)    String containing the content to be set.
     * @return array|false (mixed)  A vector of *references* to the text that match, or
     *                               FALSE on error
     * @see                 XPathEngine::wholeText()
     */
    public function _getTextSet($xPathQuery, $textPartNr = 1)
    {
        $status   = false;
     ...
     * @return array|false (array)  Vector of $absoluteXPath's (May be empty)
     * @see                setModMatch()
     */
    public function _resolveXPathQuery($xPathQuery, $function)
    {
        $xPathSet = [];
        do { // try-block
            if (isset($this->nodeIndex[$xPathQuery])) {
...
function _title($title) { 
  echo '<br><HR><b>' . htmlspecialchars($title, ENT_QUOTES | ENT_HTML5) . "</b><HR>\n";
}
...
EOD;
  ...
  // The sample code:
  $xmlOptions = [XML_OPTION_CASE_FOLDING => TRUE, XML_OPTION_SKIP_WHITE => TRUE];
