...
//
// $Id: hardware.php,v 1.1 2006/03/27 01:35:48 mikhail Exp $
//
// xml_hardware()
//
function xml_hardware()
{
    ...
    if (count($ar_buf)) {
        for ($i = 0, $iMax = count($ar_buf); $i < $iMax; $i++) {
            if ($ar_buf[$i]) {
                $pci_devices .= '      <Device>' . rtrim($ar_buf[$i]) . "</Device>\n";
            }
        }
    }
